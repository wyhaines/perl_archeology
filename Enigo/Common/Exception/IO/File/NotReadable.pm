...
=head1 FILE_NAME: $RCSfile: NotReadable.pm,v $
...
of that access was not readable.
...
package Enigo::Common::Exception::IO::File::NotReadable;
...
@Enigo::Common::Exception::IO::File::NotReadable::ISA =
  qw(Enigo::Common::Exception::IO::File);
$Enigo::Common::Exception::IO::File::NotReadable::VERSION =
...
Returns a hash blessed as a NotReadable exception.

=head2 ARGUMENTS:

Takes one, or optionally, two scalar arguments, the path of the
filesystem entity which was not readable and the value of the
...
sub new {
  ...
  return(bless Enigo::Common::Exception->new
     ({TEXT => "Error: FileNotReadable: $param->{PATH} could not be read.\n<ERROR_LOCATION/>",
       VALUE => $param->{VALUE}}),
     $self);
}
