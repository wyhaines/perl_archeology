...
use Enigo::Common::Exception::Config;
use IO::Scalar;
use Algorithm::Diff;

print "Testing Enigo::Common::Exception::Config\n\n";

my $type = 'TestError';
my $derived_type;

my $text = 'This is an error.';
my $derived_text;
...
eval
  {
    $exception = new Enigo::Common::Exception::Config
      ($type,$text);
  };
...
check('new() returns an object blessed into the correct class.',
      ref($exception) ne 'Enigo::Common::Exception::Config');
;;;;;
...
check('The expected database information is returned in the error message.',
      !($derived_type eq $type and $derived_text eq $text),
      join('',
           "Error type:\n\n",diff($derived_type,$type),"\n\n",
