...
use Enigo::Common::Exception::SQL::BadRowQuery;
use IO::Scalar;
use Algorithm::Diff;

print "Testing Enigo::Common::Exception::SQL::BadRowQuery\n\n";
...
eval
  {
    $exception = new Enigo::Common::Exception::SQL::BadRowQuery
      ('none',$sql);
  };
...
check('new() returns an object blessed into the correct class.',
      ref($exception) ne 'Enigo::Common::Exception::SQL::BadRowQuery');
;;;;;
...
check('The expected database information is returned in the error message.',
      !($derived_type eq 'row' and $derived_sql eq $sql),
      join('',
           "Error type:\n\n",diff($derived_type,'row'),"\n\n",
