...
use Enigo::Common::Exception::SQL::BadColumnQuery;
use IO::Scalar;
use Algorithm::Diff;

print "Testing Enigo::Common::Exception::SQL::BadColumnQuery\n\n";
...
eval
  {
    $exception = new Enigo::Common::Exception::SQL::BadColumnQuery
      ('none',$sql);
  };
...
check('new() returns an object blessed into the correct class.',
      ref($exception) ne 'Enigo::Common::Exception::SQL::BadColumnQuery');
;;;;;
...
$derived_sql = $2;
;;;;;
local $^W; #I just don't want to screw with stupid warnings right now.
check('The expected database information is returned in the error message.',
      !($derived_type eq 'column' and $derived_sql eq $sql),
      join('',
           "Error type:\n\n",diff($derived_type,'column'),"\n\n",
